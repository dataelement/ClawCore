...
| 🔧 **技能系统** | 可扩展的 `SKILL.md` 技能文件，渐进式暴露 —— **AI 可以自主创建和进化技能** |
...
| `skills/` | 读 + 写 | AI 可自主创建和进化技能，变更记录在 `SKILL_LOG.md` |

### 🛡️ 安全模型

ClawCore 在代码层面强制安全，而不是依赖 AI 的"自觉"：

**文件访问** — 所有文件操作都经过 `assertInsideWorkspace()` 函数，它会先解析符号链接（快捷方式）再检查路径。这是为了防止一种常见攻击：如果有人在工作区里创建了一个快捷方式，表面上看在 ClawCore 文件夹内，但实际指向的是 `/Users/你/.ssh/` 这样的敏感目录。ClawCore 会先"顺着快捷方式走到底"，发现实际位置在工作区外面，直接拒绝访问。

**Shell 命令** — `exec` 工具使用三层安全机制：

| 层级 | 做什么 | 举例 |
|------|--------|------|
| ✅ **白名单** | 安全的命令直接执行 | `ls`、`cat`、`grep`、`wc`、`open` |
| 🚫 **黑名单** | 危险命令直接拦截 | `rm`、`curl`、`wget`、`sudo`、`ssh`、`chmod` |
| ⚠️ **确认机制** | 未知命令会弹出提示，等你按 y 才执行 | `python3 script.py` → "Allow? (y/N)" |
...
| `read_skill` | 读取技能完整内容 |
| `create_skill` / `update_skill` | 创建或修改技能（自动记录到 `SKILL_LOG.md`） |
